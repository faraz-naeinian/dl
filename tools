#!/usr/bin/env bash
# Antigravity Tools LS Install Script (Linux + macOS)
# Usage: curl -fsSL https://raw.githubusercontent.com/lbjlaq/Antigravity-Tools-LS/main/install.sh | bash
#
...
APP_NAME="Antigravity Tools LS"
APP_ID="com.lbjlaq.antigravity-tools-ls"
...
build_download_url() {
    ...
    case "$PLATFORM" in
        linux)
            case "$PKG_EXT" in
                deb)
                    # Antigravity-Tools-LS_4.1.29_amd64.deb or _arm64.deb
                    DOWNLOAD_URL="${base_url}/Antigravity-Tools-LS_${RELEASE_VERSION}_${DEB_ARCH}.deb"
                    FILENAME="Antigravity-Tools-LS_${RELEASE_VERSION}_${DEB_ARCH}.deb"
                    ;;
                rpm)
                    # Antigravity-Tools-LS-4.1.29-1.x86_64.rpm or -1.aarch64.rpm
                    DOWNLOAD_URL="${base_url}/Antigravity-Tools-LS-${RELEASE_VERSION}-1.${RPM_ARCH}.rpm"
                    FILENAME="Antigravity-Tools-LS-${RELEASE_VERSION}-1.${RPM_ARCH}.rpm"
                    ;;
                AppImage)
                    # Antigravity-Tools-LS_4.1.29_amd64.AppImage or _aarch64.AppImage
                    local appimage_arch
                    ...
                    DOWNLOAD_URL="${base_url}/Antigravity-Tools-LS_${RELEASE_VERSION}_${appimage_arch}.AppImage"
                    FILENAME="Antigravity-Tools-LS_${RELEASE_VERSION}_${appimage_arch}.AppImage"
                    ;;
            esac
            ;;
        macos)
            # Prefer universal DMG, fallback to arch-specific
            DOWNLOAD_URL="${base_url}/Antigravity-Tools-LS_${RELEASE_VERSION}_universal.dmg"
            FILENAME="Antigravity-Tools-LS_${RELEASE_VERSION}_universal.dmg"
            ;;
    esac
...
install_linux() {
    ...
    case "$PKG_MANAGER" in
        ...
        appimage)
            ...
            run cp "$DOWNLOAD_PATH" "${install_dir}/antigravity-tools-ls"

            if [[ ":$PATH:" != *":${install_dir}:"* ]]; then
                warn "Add ${install_dir} to your PATH to run antigravity-tools-ls from anywhere"
